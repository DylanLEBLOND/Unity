...
namespace ProofNumberSearch
{
	public class ProofNumberSearchClass
	{
		public static int nodeCount;

		//Proof Number Search		ref : https://chessprogramming.wikispaces.com/Proof-number+search

		/// <summary>
		/// Main function of the PN search.
		/// </summary>
		/// <param name="root">Root.</param>
		/// <param name="depth">Depth.</param>
		/// <param name="isGote">If set to <c>true</c> is gote.</param>
		/// <param name="movesPlayed">Moves played.</param>
		public static Node Pns(Node root, int depth, bool isGote, List<Node> movesPlayed, ref string gameWorkflow)
		{
			List <Node> children;
			int selectedNode = 0;
			...
			Console.WriteLine ("Entree de l'algo ProofNumberSearch isGote ? {0}", isGote);
			selectedNode = 0;
			//foundNode = false;
			...
			//Console.WriteLine("Debut function");
			...
			for (int i = 0; i < children.Count; i++) {
				Console.Write (children [i].getScore());
			...
			*/
			...
			Console.WriteLine ("Sortie de l'algo ProofNumberSearch.Node Score = "+bestNode.getScore()+" Node Threat = " + bestNode.getChildrenThreat() + " | Node Checked = " + nodeCount.ToString () + " | Duree = " + (tac - tic).ToString() + " ms (tic = " + tic.ToString() + " | tac = " + tac.ToString() + ")");
			//.WriteLine ("Valeur du noeud selectionne : {0} | Threat = {1}", children[selectedNode].getValue(), children [selectedNode].getChildrenThreat());
			return bestNode;
		}
