...
namespace Tools
{
	public class Node
	{
		...
		private Token.TokenInfo _tokenInfo = new Token.TokenInfo();
		private List<Token.TokenType> _goteCapturedTokens;
		private List<Token.TokenType> _senteCapturedTokens;
		private List<Node> _children;
		private int _childrenThreat;
		private bool _isGote;
		private Node _parent = null;
		private int _index = 0;
		private int _score = 0;
		private bool  _expanded = false;
		private PNSValue _value = PNSValue.unknown;
		private int _proof = 1;
		private int _disproof = 1;
		...
		{
			if (initState.GetUpperBound (0) + 1 != 9 || initState.GetUpperBound (1) + 1 != 9)
			{
				Debug.Log ("Invalid Dim Node init [" + (initState.GetUpperBound (0) + 1) + ", " + (initState.GetUpperBound (1) + 1) + " ] ");
				return;
			}
			...
			this._state = new int[9,9];
			Array.Copy (initState, this._state, initState.Length);
		}
		...
		public Node (Node src)
		{
			...
			int [,] srcState = new int[9,9];
			src.getState (ref srcState);
			this._state = new int[9,9];
			Array.Copy (srcState, this._state, srcState.Length);
		}
		...
		{
			if (refState.GetUpperBound (0) + 1 != 9 || refState.GetUpperBound (1) + 1 != 9)
			{
				Debug.Log ("Invalid Dim getState [" + (refState.GetUpperBound (0) + 1) + ", " + (refState.GetUpperBound (1) + 1) + " ] ");
				return;
			}

			Array.Copy (this._state, refState, this._state.Length);
		}
		...
		public void setState (ref int [,] refState)
		{
			if (refState.GetUpperBound (0) + 1 != 9 || refState.GetUpperBound (1) + 1 != 9)
			{
				Debug.Log ("Invalid Dim setState [" + (refState.GetUpperBound (0) + 1) + ", " + (refState.GetUpperBound (1) + 1) + " ] ");
				return;
			}

			Array.Copy (refState, this._state, refState.Length);
		}

		public void showState ()
		{
			Debug.Log (this.getStateInString());
		}
		...
		public int evaluate (bool isGote)
		{
			...
			if ((isGote && !this.stillAlive (isGote)) || (!isGote && !this.stillAlive (!isGote)))		// The current player lost
				return -999999999;

			if ((isGote && !this.stillAlive (!isGote)) || (!isGote && !this.stillAlive (isGote)))		// The current player won
				return 999999999;
						...
						switch (this._state [i, j])
						{
							...
							default:
								if (this._state[i,j] != Token.KING)
									Debug.Log ("Gote Unknown Token state[" + i + "," + j + "] = " + this._state[i,j]);
								break;
						}
					}
					else if (this._state [i, j] < 0)
						...
						switch (this._state [i, j])
						{
							...
							default:
								if (this._state[i,j] != Token.JEWEL)
									Debug.Log ("Sente Unknown Token state[" + i + "," + j + "] = " + this._state[i,j]);
								break;
						}
					}
				}
				...
				for (int y = 0; y < 9; y++)
				{
					...
					for (int x = 0; x < 9; x++)
					{
						if (this._state[y, x] == 0)
						{
							...
							Array.Copy (this._state, nextState, this._state.Length);
							nextState [y, x] = token.newValue;
